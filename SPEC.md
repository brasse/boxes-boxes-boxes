...
one up — type a word, see the item, its box number and where that box is — and filing one:
a single object that needs a home, recorded and photographed on the spot. This is the flow
that continues indefinitely once the initial cataloguing is done.
...
|---|---|---|
| **box-inventory** | to create | The application. Everything in this spec unless stated otherwise. |
| **pylib** | **does not exist — create it** | Reusable components, shared across personal projects. Initially just the blob store. |
...
> §2.2–§2.3 and in import statements.
...
Core over SQLite, an abstract database class, a blob store — and is worth reading for
...
pylib/
  ...
  src/pylib/blobstore/__init__.py        # re-exports BlobStore, BlobNotFoundError
...
**Primary configuration** in `box-inventory/pyproject.toml`:
...
```
python -m box_inventory.regenerate_images [--variant full|thumb] [--dry-run]
```
...
```
python -m box_inventory.set_password <username> [--password <pw>]
```
...
what the outside world sees, because it is unguessable, not *enumerable* — nobody can walk
`/items/1`, `/items/2` — and doesn't leak how many rows exist or in what order they were
created.
...
- **A `413` body may never reach the client.** If the server rejects an oversized upload
  ...
  With 3–5 MB originals (§4) this is realistic, so the frontend must treat a failed upload
...
typing. This application has **one user, one process, one machine** — none of those
...
Timestamps are stored as RFC 3339 UTC strings — SQLite has no native datetime type.
...
items legitimately share a key — photographing six identical storage bins once and
attaching that photo to each, for instance. Deduplication is a *consequence* of content
...
```
python -m box_inventory.gc [--dry-run]
```
...
It emits types only — no runtime, no client abstraction to learn — and is paired with a
...
- **Author CSS narrow-first** — base styles target the phone, a single
...
- Tags persist between entries by default — consecutive items in one box usually share
  them — and are individually removable.
...
the tailnet. This is already a working phone experience — it is not a
placeholder for one.
...
| Backup token | `backup_token` | `BACKUP_TOKEN` | Token for `POST /api/admin/backup`. Endpoint disabled if unset. |
| Cookie security | `session_cookie_secure` | `SESSION_COOKIE_SECURE` | Default `false`; `true` in this deployment. |
| Data directory | — | `DATA_DIR` | Default `./data`; the Dockerfile sets `/data`. |
...
`python -m box_inventory.set_password <username>`. They are therefore covered by the
...
*`session_cookie_secure` is `true`.* This is not a requirement on the app — it is one word
...
- **Setting a password is an explicit step, not magic.** A fresh install has no users. The
  ...
  instruction to run `python -m box_inventory.set_password <username>`. No password is
...
- **Search, treated as a behavioural contract rather than an implementation test.** §6.3
  ...
  true is a suite asserting *behaviour* — this query finds that item, filters compose, `tag`
  is conjunctive, results come back in server order — which then runs unchanged against the
  new implementation. Without it the upgrade path is a promise, not something verifiable.
...
Note there is nothing to test about content verification or write-once refusal — the store
does neither, by design.
...
The `pylib` half is the part that earns its keep. `box-inventory` pins a `pylib` git tag
...
0. **Create the `pylib` repository** and extract the blob store into it (§2.2) with the
   ...
   `box-inventory` can depend on it (§2.3). This comes first because image handling depends
   on it.
...
pass. Deliberately not milestone 1 — the photo pass already ensures every item gets a
